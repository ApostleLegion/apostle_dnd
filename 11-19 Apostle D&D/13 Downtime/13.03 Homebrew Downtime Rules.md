...
Crafting mundane items is a lot easier than crafting magical ones. A player who wishes to craft a mundane item can do so with the crafting downtime. If a player is not proficient in the tools necessary for crafting the item they pay full price in gold for the item and do not need to roll. If a player is proficient in the correct tools they may reduce the cost by 10% of its original value and craft it. If a player has expertise in the correct tools this discount rises to 25% of the total price. 

# Updated Crafting Rules 
For items that have an incremental upgrade (IE +1, +2, etc) the player that crafted the previous item and wishes to craft the next upgrade of the item gets a bonus of their proficiency modifier added on top of all other modifiers the crafting roll may receive. In addition, a session of crafting is reduced from the number needed to complete the item in tandem with the +x bonus. Meaning a +2 to a +3 would reduce the required successful crafting sessions to four from six for a very rare item. The character that wishes to “upgrade” their item still needs to get the recipe of the new item version but also gets their proficiency bonus added to the research roll for Lore for the recipe of the +x item upgrade. Items that are upgraded are not new items meaning you do not get a +1 and +2 if you upgrade your +1.
Item Disassembling, DC is the same as crafting the item, get at maximum 50% of the shards back (no gold) and can lose additional shards if the roll is botched (to a minimum of 1 shard given back)
Specific Item crafting (ie dragon wrath etc.) now needs to get a part of the creature necessary using a bounty.
Challenges for said weapon to increase its rarity and still need full shards but no time or gold. For normal +1 to +3 magic items challenge can be completed to half the time, zero the gold, but still max shards for the item to be upgraded.
Challenges will be specific to items so talk to a DM/Admin about them.
Duo Crafting: XP is given to the second person if they RP, and 1 lore is automatically given to the second player.
Crafting Consecutive Fail Bonus: If a player fails to craft the same item two times the next time they try to craft the item they get a +2 to that roll. If they consecutively fail after that bonus, they get +2 added on for every fail after (+2, +4, +6, etc)
